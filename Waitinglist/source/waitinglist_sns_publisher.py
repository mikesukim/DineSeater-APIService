...
from source.constant_variables import TOPIC_ARN_PREFIX,STAGE,TEST_PHONE_NUMBERS,PUSH_NOTIFICATION_TITLE,PUSH_NOTIFICATION_BODY

class WaitinglistSNSPublisher:
    def __init__(self, sns_client):
        self.sns_client = sns_client
    
    def publish_waiting(self, business_name, new_waiting):
        topic_arn = TOPIC_ARN_PREFIX + business_name
        print("sns is publishing new waiting: ", new_waiting)
        print(new_waiting)
        message = self.create_fcm_message(PUSH_NOTIFICATION_TITLE, PUSH_NOTIFICATION_BODY, new_waiting)
